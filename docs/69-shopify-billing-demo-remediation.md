...
Actual **Open theme editor** reproduced a blank/failed embedded document. First patch `5ec8983b84633066d955c30130e798a720119e67` added `target="_top"` to both React Router forms; focused12/12 tests, TypeScript, targeted lint and production build passed. It was pushed/deployed as healthy Fly release47/image `deployment-01M2DKXVGC49C0D3TCGPT2TF8S`, with28 current migrations and encrypted backup/isolated restore1881ms. Live retest still failed despite a DOM-confirmed `_top` form, so this repair is NOT accepted. Direct top-level navigation is being corrected and requires actual browser proof.

Pre-final-release observations: real synthetic product shows Original, USD10, intact native Add to cart/Buy it now and disclosure; adaptive panel hidden with `KILL_SWITCH_ACTIVE`, measured=false, no deployment. Desktop and390x844 responsive views are readable. Native cart contains only the agent-added one pouch/USD10; standard checkout opens with the visible **Test Payment Gateway**, contact/delivery/payment controls and USD10 Pay now. No payment or order was submitted. Browser Back returned to the cart; root removed only its added item and confirmed **Your cart is empty**. Temporary viewport override was reset. These observations are not authenticated current-release QA receipts and do not certify completed checkout, denied-consent behavior, Shop Pay or performance. Fresh final-release captures and exact artifact review remain required.

Final navigation repair `4acebb80ad7a87eb52fd90c0aef31d3321294fb2` replaces enhanced forms with direct top-level anchors. Render-level regression, focused12/12 tests, TypeScript, targeted lint and root production build pass. It is pushed/deployed as healthy Fly release48/image `deployment-01M2DMABXYGM6ZHEYRDMJ4Q2RW`;28 migrations current and encrypted backup/isolated restore1596ms verified. Actual link opens the real Shopify editor. Shopify's add-block link proposed a duplicate and opened Gift Card; root reverted exactly its unsaved addition, verified one block/Save disabled and selected the synthetic pouch using the native product preview selector. No published theme change was saved.

After an actual current-release synthetic product load, Overview **I saved it — verify now** returned **Published theme verified** and plan VERIFYING. Authenticated current-release placement/original_fallback receipts pass; exact owner approval remains unchanged. Active experiments/deployments0 and recovery hold ON. Desktop1728x804/mobile390x844 screenshots, standard cart-to-checkout and Buy it now-to-checkout were recaptured on this release; no payment/order was submitted. Final agent-added cart item was removed and empty cart confirmed. Shop Pay is not configured: actual Payments page shows test gateway and Shopify Payments Complete setup. No payment/privacy setting was changed. Seven formal manual QA checks remain unaccepted; exact artifacts, limits and consent/performance blockers are in doc70. The owner has not yet answered permission to enable test1's cookie banner for US visitors. No missing check, baseline activation or public rollout was approved by inference.
...
The subsequent Overview reload exposed a real recovery gap: the existing selected-product Retry UI handles invalidated legacy plans, not this already-v2 invalidated plan. Generic preparation is blocked by the retained cutover receipt. A bounded explicit **Recover after reinstall** action preserves the old immutable history, creates a newly linked receipt/preparation, and requires fresh plan approval. No direct database plan resurrection or implicit approval is permitted. This supersedes the unanswered-pause notes below.

The recovery implementation adds an authenticated owner/operator action limited to the selected shop. It verifies installed session/active pixel, exact invalidated v2 plan and historical receipt lineage, already-cleared runtime and no active experiment/deployment. A new linked immutable receipt restores the selected-product preparation hold; the ordinary receipt-bound preparation path produces a distinct unapproved plan. Exact replay reuses the receipt, and neither the old plan nor its approval is rewritten. Focused cutover tests11/11, TypeScript, targeted ESLint and diff validation pass. Deployment and live recovery outcome are recorded separately below.

Actual first recovery attempt after source `b0eae615` and exact test1 cutover configuration returned `V2_CUTOVER_SOURCE_CHANGED`; no recovery receipt or plan was created. Current catalog version `2026-09-06T19:53:50Z` differs from the old receipt's `2026-09-06T09:42:31Z`. The follow-up fix treats that prior snapshot only as immutable lineage and binds the new receipt to the current catalog shown at action time. Shopify is synchronized before recording; any change since display aborts with `V2_REINSTALL_SOURCE_CHANGED`. Regression deliberately changes source/document/evidence versions, rejects a stale displayed snapshot, and proves a distinct unapproved READY plan through the existing preparation function. It does not claim queue-worker or browser success. Focused11/11, TypeScript, targeted ESLint, root diff review and production build pass. No experiment activation is part of recovery.

Source `db79aedad84edbbf86f3cdebf2988c680ae1d4f4` is pushed and deployed as Fly release46/image `deployment-01M2CK48VFWNY14R0656CJBKBC`. Public health and Fly checks pass after startup;28 migrations remain current and encrypted backup verification passed. Actual browser Recover returned **Post-reinstall recovery recorded. Fresh source-backed preparation is queued and requires a new approval.** New receipt `cmtzd7f0k007kq6lafgyydxtc` links prior `cmtpmvlu500f3q6lbj336xp2g` and binds current source `2026-09-06T19:53:50Z` / `d931f97e…c59f`. Initial read-only worker snapshot: job `cmtzd7f0r007qq6la5xb8jy1u` PENDING, attempts0, no error/result. This is queued recovery, not completed preparation. The new selected-product preparation hold is ON; old invalidated plan and its Sep6 approval remain preserved; active experiments0 and deployments0. Normal worker dispatch is being checked before any further claim.

Post-cycle outcome: no dispatch defect. Production maintenance runs every five minutes; receipt creation at05:19:55UTC followed the05:19:10 cycle. The next normal cycle completed job `cmtzd7f0r007qq6la5xb8jy1u` in one attempt with no error; no manual maintenance or extra alert reconciliation was triggered. Fresh plan `cmtzdcx1r00m1q6larwx71jys`, version2, is READY_FOR_APPROVAL, bound to the new receipt/current source, with approvedAt/approvedBy/approvalRecordJson all null. The historical invalidated plan and original approval/hash remain unchanged. Active experiments0; active deployments0. Actual Overview displays **Review one product opportunity**, both Universal cards and **Approve and prepare test**. Root did not click approval or attest the medium-risk checkbox. Current headlines are **One pouch is included in each test order.** and **Two internal pockets separate cables and adapters.** The supporting bullets retain the explicit synthetic-fixture/no-real-item disclosures and sourced zip-closure, pockets and dimensions facts. Fresh owner review/approval is the next authority boundary; it is not covered by permission to clear an uninstall pause. Scoped QA, activation, final actual demo and resubmission remain incomplete.
...
Bounded verification: 19/19 measurement tests, TypeScript and production build passed. Root reviewed the change and deployed source `e8ed09a6c0f5cb7bee4e1929c5bb5ca9ecd2feb8` as release42/image `deployment-01M2BKBK3R644CTNHASYKCAZXC`. Actual measurement-page reload recovered the pixel; **Reconnect pixel** then returned success with a rotated credential. The persisted ACTIVE pixel exactly matches Shopify's canonical `gid://shopify/WebPixel/2710405426`, activated20:04:37UTC. Public health200 and Fly check pass; all28 migrations are current. Startup encrypted backup `pagnetic-7fb5f1fc-2f9e-4559-82c2-c79a322743da.sqlite.enc` verified/restored in1979ms without changing remote objects. Fly's local DNS probe timed out, but the public health request succeeded.

The uninstall kill switch and privacy cleanup count10 remain intact. Normal preparation recovery needs the explicit Operations workflow because uninstall invalidates the old plan; no direct database hold clearing is permitted. All recordings are stopped/downloaded and the local recorder server is stopped. No Shopify finding was marked resolved or resubmitted in this continuation.

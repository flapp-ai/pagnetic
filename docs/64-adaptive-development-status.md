...
Post-deploy production verification passes every readiness check except external alert delivery because `ALERT_WEBHOOK_URL` is still absent. Runtime verification confirms billing, v2 serving, shadow serving, model use and offer publication remain `false`. The current extensions/configuration were built, validated and released through Shopify as `pagnetic-ap06-20260906` (version ID `1117888282625`). No treatment, charge, App Store publication, design-partner approval or lift claim is implied.
